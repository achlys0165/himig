...
const SetlistPage: React.FC = () => {
  ...
  const goToNextSong = () => {
    const songs = currentCategorySongs;
  ...
  const goToPrevSong = () => {
    if (currentSongIndex > 0) {
      ...
      setSelectedSong(currentCategorySongs[prevIndex]);
    }
  };

  // Swipe handlers for mobile
  const swipeHandlers = useSwipe(goToNextSong, goToPrevSong);
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4">
      ...
      {selectedSong && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
          <div 
            className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl"
            {...swipeHandlers}
          >
            {/* Navigation Header */}
            <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
              <div className="flex items-center gap-2 pointer-events-auto">
                <button 
                  ...
                  className="p-2 bg-white/10 hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed rounded-full transition-all backdrop-blur-md"
                >
                  <ChevronLeft size={20} className="text-white" />
                </button>
                <span className="text-xs font-bold text-white/60 uppercase tracking-widest">
                  {currentSongIndex + 1} / {currentCategorySongs.length}
                </span>
                <button 
                  onClick={goToNextSong}
                  disabled={currentSongIndex === currentCategorySongs.length - 1}
                  className="p-2 bg-white/10 hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed rounded-full transition-all backdrop-blur-md"
                >
              ...
              <div className="flex items-center gap-2 pointer-events-auto">
                <span className="text-[10px] font-black uppercase tracking-widest text-white/40 bg-white/10 px-3 py-1.5 rounded-full">
                  {currentCategory}
                </span>
                <button 
            ...
            <div className="absolute top-16 left-0 right-0 flex justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity md:hidden">
              <span className="text-[10px] text-white/30 uppercase tracking-widest bg-black/50 px-3 py-1 rounded-full">
                Swipe to navigate
              </span>
            </div>
            ...
            <div className="p-4 border-t border-white/5 bg-black/50 backdrop-blur-md flex items-center justify-between">
              <div className="flex items-center gap-4">
                <button 
                  onClick={goToPrevSong}
                  disabled={currentSongIndex === 0}
                  className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed rounded-full transition-all"
                >
                  <ChevronLeft size={16} />
              ...
              <div className="text-center hidden sm:block">
                ...
                <p className="text-[10px] text-white/20">
                  {currentCategory} • {currentSongIndex + 1} of {currentCategorySongs.length}
                </p>
              </div>
